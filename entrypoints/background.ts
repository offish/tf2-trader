export default defineBackground(() => {
  ...
  browser.runtime.onMessage.addListener(
    (
      ...
      const msg = message as { type?: string; sku?: string; query?: string; baseSku?: string };
      ...
      if (msg?.type === "pricedb_search" && msg.query) {
        ...
        fetch(`https://pricedb.io/api/search?q=${encodeURIComponent(q)}&limit=5`)
          .then(async (res) => {
            if (!res.ok) { sendResponse(null); return; }
            const data = await res.json();
            const results: any[] = data?.data?.results ?? [];
            if (results.length === 0) { sendResponse(null); return; }
            // Prefer the first result that has a series suffix (;cNNN) in its SKU.
            // Fall back to the first result if none has one.
            const withSeries = results.find((r: any) => typeof r.sku === "string" && /;c\d+/.test(r.sku));
            const item = withSeries ?? results[0];
